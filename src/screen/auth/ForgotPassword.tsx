...
const ForgotPassword = () => {
    ...
    const sendOtp = async () => {
        ...
        let isValid = true, newForm = { ...form }
        if (!newForm.mobileNumber) newForm.mobileNumberError = 'Required Registerd Mobile Number ', isValid = false
        if (newForm.mobileNumber && !newForm.mobileNumber.match(/^(?:\+\d{1,3}\s?)?(?:\d{3}|\(\d{3}\))[\s.-]?\d{3}[\s.-]?\d{4}$/)) newForm.mobileNumberError = 'Valid Mobile Number is required', isValid = false
                ...
                if (res?.data?.statusCode === 200) {
                    ...
                    if (res?.data?.response?.token) {
                        ...
                        navigation.dispatch(
                            CommonActions.reset({
                                index: 0,
                                routes: [{ name: 'ForgotPasswordOtpVerify', params: { resetToken: res?.data?.response?.token } }],
                            })
                        );
                    }
                }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.logoAlign}>
                <Image source={IMAGES.forgotPassword} style={styles.forgotPasswordLogo} />
            </View>
            <Text style={styles.otpText}>Kindly enter registered mobile number.</Text>
            ...
            <View style={styles.inputRow}>
                ...
                <View style={{ width: "70%" }}>
                    <TextInputCustom
                        ...
                        activeIcon={toogleFunc}
                        maxLength={10}
                        onChangeText={(mobileNumber) => setForm({ ...form, mobileNumber, mobileNumberError: "" })}
                        label="Number" />
                </View>
            </View>
